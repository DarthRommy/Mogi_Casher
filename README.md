...
### バーコードどうやって入れてんの？
バーコードリーダは読み取ったときにEnterを送信するらしい、という情報を聞いた。  
ちょうどPySimpleGUIのWindowオブジェクトには"return_keyboard_events"という引数があり、Trueにするとキーボード入力をイベントとして処理してくれる。  
更に、カーソルがInput欄にあっている間にバーコードを読むと、要はキーボードで数字を打ち込んだのと同じなので、読んだ値が入力される。  
つまり、Enter="/r"イベントが発生したときにInput欄の値を判定するようにしたわけですね。

### シンプルなソフトとは
シンプルとか言うといてなに6つもファイル分かれてんねん！と思ったやろ？  
建前としては今後いじりやすいようにやねんけど、実際は挙動を謎に作りこんだからなんですよね
- レイアウトを切り替えられるようにしてみたり  
Pythonでモダンなソフトを作るとか狂気の沙汰やろ
- 流行りのダークモードを実装してみたり  
ダークモードの反映のための再起動の機能を付けたせいで[余計な関数](/mogicasher/mogicasher.py#L42)が一つ増えてしまった
